...
LightController::LightController() {
	...
	lightIDMap.reserve(MAX_NUM_LIGHTS*3); // Magic number "3" because I never plan on having more than 3 light types...
...
void LightController::DrawLights() {
	for_each(pointLights.begin(), pointLights.end(), [](PointLight* &n) { n->draw(); LOG(INFO) << "Point light drawn."; });
	for_each(spotLights.begin(), spotLights.end(), [](SpotLight* &n) { n->draw(); LOG(INFO) << "Spot light drawn."; });
	for_each(directionalLights.begin(), directionalLights.end(), [](DirectionalLight* &n) { n->draw(); LOG(INFO) << "Directional light drawn."; });
}
...
int LightController::InsertLight(
	...
	case LIGHT_POINT:
		...
		LOG(INFO) << "Inserted point light. ID: " << pointLights.size() - 1;
		LOG(INFO) << "constantIn, linearIn, exponentIn: " << constantIn << " " << linearIn << " " << exponentIn;
	...
	case LIGHT_DIRECTIONAL:
		...
		LOG(INFO) << "Inserted directional light.";
		break;
